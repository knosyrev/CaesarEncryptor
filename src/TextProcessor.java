...
public class TextProcessor {
    ...
    private static final int LETTER_AMOUNT_RU = ALPHABET_RU.length; //amount of letters in russian alphabet
    ...
    public static char processLetter(Character initialLetter, int key) {
        return (char) (ALPHABET_RU[0] + (initialLetter - ALPHABET_RU[0] + key) % LETTER_AMOUNT_RU);
    }

    public static String statisticalDecryption(String text) throws IOException {
        Map<Character, Integer> charFrequency = getCharFrequency(text);            //get frequency of every character in text
        int key = getMostFrequentCharKey(charFrequency);                           //get key according to the frequency
    ...
    public static String bruteForce(String text) {
        ...
        for (int key = 1; key < LETTER_AMOUNT_RU; key++) {                     //iterate over key value
    ...
    public static int getLetterAmount() {
        return LETTER_AMOUNT_RU;
    }
